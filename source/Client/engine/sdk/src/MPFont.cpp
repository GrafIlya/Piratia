...
#endif
{
	...
	SIZE size;
	::GetTextExtentPoint(_hDc, "å¥½", 1, &size);
...
//	Çàêîìåíòèðîâàíî ïî ïðè÷èíå íåêîðåêòíîãî îòîáðàæåíèÿ ðóññêîãî ÿçûêà by Jasper
		/*if (*ch & 0x80) {
			ch++;
			offset = w * 2 + ASSIZE;
		} else*/ 
...
bool CMPFont::TextToTexture(char c1, char c2, float& tX, float& tY) {
	...
	if (it == _vecBuf.end()) {
		...
		::TextOut(_hDc, 10, -2, sz, c1 & 0x100 ? 2 : 1);//Ôèêñàöèÿ âûñîòû íàïèñàíèÿ òåêñòà by Jasper
	...
	/*	if (ch[0] & 0x80) {
...

		{
			...
			if (!GetIntersectRect(&dest, &rc, pclip)) {
				sx += offset;
				continue; //å¦‚æžœè¿™ä¸ªå­—ä¸åœ¨è£åˆ‡åŒºï¼Œä¸æ˜¾ç¤º
			} else if (rc.left >= pclip->left && rc.right <= pclip->right && rc.top >= pclip->top && rc.bottom <= pclip->bottom) {
				if (bExit) {
				...
				//å¦‚æžœè¿™ä¸ªå­—å…¨éƒ¨åœ¨è£åˆ‡åŒºï¼Œæ˜¾ç¤º
			} else {
				//è£åˆ‡
				tx = (float)dest.left;
				ty = (float)dest.top;
...
//	Çàêîìåíòèðîâàíî ïî ïðè÷èíå íåêîðåêòíîãî îòîáðàæåíèÿ ðóññêîãî ÿçûêà by Jasper
		...
		if (it == _vecBufFast.end()) {
			...
			::TextOut(_hDc, 0, -2, sz, ch[0] & 0x100 ? 2 : 1);//Ôèêñàöèÿ âûñîòû íàïèñàíèÿ òåêñòà by Jasper
