...
#endif
{
	...
	SIZE size;
	::GetTextExtentPoint(_hDc, "еҐЅ", 1, &size);
...
//	Закоментировано по причине некоректного отображения русского языка by Jasper
		/*if (*ch & 0x80) {
			ch++;
			offset = w * 2 + ASSIZE;
		} else*/ 
	...
	/*	if (ch[0] & 0x80) {
...

		{
			...
			if (!GetIntersectRect(&dest, &rc, pclip)) {
				sx += offset;
				continue; //е¦‚жћњиї™дёЄе­—дёЌењЁиЈЃе€‡еЊєпјЊдёЌжѕз¤є
				...
				//е¦‚жћњиї™дёЄе­—е…ЁйѓЁењЁиЈЃе€‡еЊєпјЊжѕз¤є
			} else {
				//иЈЃе€‡
				tx = (float)dest.left;
				ty = (float)dest.top;
