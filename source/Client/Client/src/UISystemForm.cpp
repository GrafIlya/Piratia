...
CSystemProperties::SVideo::SVideo() : bFullScreen(false), bPixel1024(false), nTexture(0), nQuality(0),
...
bool CSystemMgr::Init() {
	...
	if (!cbxTexture)
		return Error("msgui.clu界面<%s>上找不到控件<%s>", frmVideo->GetName(), "cbxTexture");
	...
	if (!cbxMovie)
		return Error("msgui.clu界面<%s>上找不到控件<%s>", frmVideo->GetName(), "cbxMovie");
	...
	if (!cbxCamera)
		return Error("msgui.clu界面<%s>上找不到控件<%s>", frmVideo->GetName(), "cbxCamera");
	...
	*/
	...
	if (!cbxTrail)
		return Error(g_oLangRec.GetString(45), frmVideo->GetName(), "cbxTrail");
	...
	if (!cbxColor)
		return Error(g_oLangRec.GetString(45), frmVideo->GetName(), "cbxColor");
	...
	if (!cbxSize)
		return Error(g_oLangRec.GetString(45), frmVideo->GetName(), "cbxSize");
	...
	if (!cbxModel)
		return Error(g_oLangRec.GetString(45), frmVideo->GetName(), "cbxModel");
	...
	if (!cbxQuality)
		return Error(g_oLangRec.GetString(45), frmVideo->GetName(), "cbxQuality");
	...
	if (!proAudioMusic)
		return Error(g_oLangRec.GetString(45), frmAudio->GetName(), "proAudioMusic");
	...
	if (!proAudioMidi)
		return Error(g_oLangRec.GetString(45), frmAudio->GetName(), "proAudioMidi");
	...
	if (!frmGameOption)
		return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "frmGame");
	...
	if (!cbxRunMode)
		return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxRunmodel");

	cbxLockMode = (CCheckGroup*)frmGameOption->Find("cbxLockmodel");
	if (!cbxLockMode)
		return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxLockmodel");

	cbxHelpMode = (CCheckGroup*)frmGameOption->Find("cbxHelpmodel");
	if (!cbxHelpMode)
		return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxHelpmodel");

	cbxAppMode = (CCheckGroup*)frmGameOption->Find("cbxAppmodel");
	if (!cbxAppMode)
		return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxAppmodel");

	cbxEffMode = (CCheckGroup*)frmGameOption->Find("cbxEffmodel");
	if (!cbxEffMode)
		return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxEffmodel");
