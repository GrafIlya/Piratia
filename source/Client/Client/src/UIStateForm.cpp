...
bool CStateMgr::Init() {
	...
	if (!labStateJob)
		return Error(g_oLangRec.GetString(45), frmState->GetName(), "labStateJob");

	labStateLevel = dynamic_cast<CLabelEx*>(frmState->Find("labStateLevel"));
	if (!labStateLevel)
		return Error(g_oLangRec.GetString(45), frmState->GetName(), "labStateLevel");

	labStatePoint = dynamic_cast<CLabelEx*>(frmState->Find("labStatePoint"));
	if (!labStatePoint)
		return Error(g_oLangRec.GetString(45), frmState->GetName(), "labStatePoint");

	labSkillPoint = dynamic_cast<CLabelEx*>(frmState->Find("labSkillPoint"));
	if (!labSkillPoint)
		return Error(g_oLangRec.GetString(45), frmState->GetName(), "labSkillPoint");
	...
	if (!btnAgi)
		return Error(g_oLangRec.GetString(45), frmState->GetName(), "btnAgi");
	btnAgi->evtMouseClick = MainMouseDown;

	btnCon = dynamic_cast<CTextButton*>(frmState->Find("btnCon"));
	if (!btnCon)
		return Error(g_oLangRec.GetString(45), frmState->GetName(), "btnCon");
	btnCon->evtMouseClick = MainMouseDown;

	btnSta = dynamic_cast<CTextButton*>(frmState->Find("btnSta"));
	if (!btnSta)
		return Error(g_oLangRec.GetString(45), frmState->GetName(), "btnSta");
