...
*/
	...
	try {
		...
		//LG("group_sql", "TBLCharacters::FetchRowByChaID??SQL, ????!,cha_id =%d\n", cha_id);
...
bool TBLCharacters::BackupRow(int cha_id) {
	...
	if (l_retrow == 1) {
		...
		//LG( "????", "BackupRow:????,??????????!???sql??.cha_id = ", cha_id );
	...
	sprintf(sql, "update %s set delflag =1,deldate =getdate() where cha_id=%d", _get_table(), cha_id); //  ??????
	SQLRETURN l_ret = Exec_sql_direct(sql, this);
	return (DBOK(l_ret)) ? true : false;
}
...
bool TBLGuilds::Disband(uLong gldid) {
	...
	if (!DBOK(l_sqlret)) {
		if (DBNODATA(l_sqlret)) {
			...
			//l_line<<newln<<"????SQL????2!??ID:"<<gldid;
			l_line << newln << "dismiss guild SQL failed2! guild ID:" << gldid;
			return false;
		} else {
			LogLine l_line(g_LogGuild);
			//l_line<<newln<<"????SQL????1!??ID:"<<gldid;
	...
	if (disband_days < 1) {
		...
		*/
		sql_syntax =
			...
			string(", case when g.stat>0 then %d*24*60 -DATEDIFF(mi,g.disband_date,GETDATE()) else 0 end LeaveMins ") + string(",g.level from guild As g where (g.guild_id >0) ");
	}
